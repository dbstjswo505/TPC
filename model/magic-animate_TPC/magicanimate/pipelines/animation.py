    ...
    *_, func_args = inspect.getargvalues(inspect.currentframe())
    ...
    for idx, (source_image, test_video, random_seed, size, step) in tqdm(
        ...
        if test_video.endswith('.mp4'):
            _,pose_name = os.path.split(test_video)
            pose_name = pose_name[:-4]
        ...
        if source_image.endswith(".mp4"):
            ...
            _,ref_name = os.path.split(source_image)
            ref_name = ref_name[:-4]
            tmp = Image.open(source_image).convert('RGB')
            source_image = np.array(tmp.resize((size, size)))
            #source_image = np.array(Image.open(source_image).resize((size, size)))
        H, W, C = source_image.shape
        
        print(f"current seed: {torch.initial_seed()}")
        init_latents = None
        # sj calibrated ref image branch: L x 512 x 512 x 3
        ...
        if args.rank == 0:
            ...
            if config.use_pad:
                ...
                pose_mask = []
                for e in range(3):
                    ppath = f'/mnt/hdd/sunjaeyoon/workspace/Neurips2024/magic-animate/inputs/applications/calibrated_image/pose_{pose_name}_ref_{ref_name}/{e}-person_pose_mask_img'
                    if os.path.isdir(ppath):
                        ...
                        for i in range(len(flst)):
                            pmask_name = os.path.join(ppath,flst[i])
                            tmp_ = Image.open(pmask_name).convert('RGB')
                            tmp = tmp + [np.array(tmp_.resize((W, H)))]
                        tmp2 = np.stack(tmp, axis=0)
                        #pose_mask_ = np.pad(tmp2, ((0, context_frames - tmp2.shape[0] % context_frames), (0, 0), (0, 0), (0, 0)), mode='edge')
                        pose_mask = pose_mask + [torch.from_numpy(tmp2)]
                pose_mask = torch.cat(pose_mask, dim=0)
                pose_mask = pose_mask[:,:,:,0]
                tmp = np.expand_dims(pose_mask, axis=0)
                pose_mask = np.expand_dims(tmp, axis=1)
                pose_mask = pose_mask > 0.1
                for v in range(len(flst)):
                    pose_mask[0,0,v] = ndimage.binary_dilation(pose_mask[0,0,v], 
                            [[True, True, True],
                            [ True, True, True],
                            [ True, True, True]], iterations=2,
                            )
                sample[:,:,:original_length] = sample[:,:,:original_length] * pose_mask + source_images * ~pose_mask
