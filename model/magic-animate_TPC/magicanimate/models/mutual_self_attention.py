...
class ReferenceAttentionControl():
    ...
    def register_reference_hooks(
        ...
        def cal_mask_generate(X, Y, step, fd, raw_mask, p_mask_, tp, cp):

            B, Np, D = X.shape
            B, Ni, D = Y.shape
            if D == 1280 or D == 640 or D == 320:
                # sam mask based attntion mask
                c_mask = raw_mask[:,tp:,:,:]
                c_mask = c_mask.view(2,tp,-1,512,512)
                c_mask = c_mask[:,cp,:,:,:] # c_mask: 2 x total_video_length x 512 x 512
                c_mask = c_mask[:,torch.tensor(fd),:,:] 
                
                p_mask = p_mask_
                p_mask = p_mask.view(2,tp,-1,512,512)
                p_mask = p_mask[:,cp,:,:,:]
                p_mask = p_mask[:,torch.tensor(fd),:,:]
                
                w = int(512/Np**(0.5))
                m = torch.nn.AvgPool2d(w, stride=w)
                
                # 2 x 16 x 8 x 8
                thr = 255*0.001
                p_mask = m(p_mask.to(torch.float64)).view(2,len(fd),-1)
                c_mask = m(c_mask.to(torch.float64)).view(2,len(fd),-1)
                p_mask = p_mask > thr
                c_mask = c_mask > thr
                p_mask = p_mask.unsqueeze(-1)
                c_mask = c_mask.unsqueeze(-1)

                pc_mask = torch.einsum('bnmd,bnld->bnml', p_mask.to(torch.float16), c_mask.to(torch.float16))
                pc_mask = pc_mask.view(-1,Np,Np)
                mask = torch.zeros((B,Np,Np), dtype=X.dtype, device=X.device)
                mask[:,:,:] = (1 - pc_mask[:,:,:]) * -10000.0

                return mask
        
        def ref_mask_generate(X, Y, step, fd, raw_mask, p_mask_, tp, cp):

            B, Np, D = X.shape
            B, Ni, D = Y.shape
            if D == 1280 or D == 640 or D == 320:
                # sam mask based attntion mask
                r_mask = raw_mask.to(torch.float64)
                r_mask = r_mask[:,:tp,:,:]
                r_mask = torch.mean(r_mask, dim=1, keepdim=True) 
                r_mask = r_mask.repeat(1,len(fd),1,1) # B, L, 512, 512
                w = int(512/Np**(0.5))
                m = torch.nn.AvgPool2d(w, stride=w)
                
                p_mask = p_mask_.to(torch.float64)
                p_mask = p_mask.view(2,tp,-1,512,512)
                p_mask = p_mask[:,:,torch.tensor(fd),:,:]
                p_mask = torch.mean(p_mask, dim=1) # B, L, 512, 512
                
                # 2 x 16 x 8 x 8
                thr = 255*0.001
                r_mask = m(r_mask).view(2,len(fd),-1)
                r_mask = r_mask < thr
                r_mask = r_mask.unsqueeze(-1)

                p_mask = m(p_mask).view(2,len(fd),-1)
                p_mask = p_mask < thr
                p_mask = p_mask.unsqueeze(-1)

                pr_mask = torch.einsum('bnmd,bnld->bnml', p_mask.to(torch.float16), r_mask.to(torch.float16))
                pr_mask = pr_mask.view(-1,Np,Np)
                mask = torch.zeros((B,Np,Np), dtype=X.dtype, device=X.device)
                mask[:,:,:] = (1-pr_mask[:,:,:]) * -10000.0
        ...
        def hacked_basic_transformer_inner_forward(
            ...
            if self.only_cross_attention:
                ...
                if MODE == "read":
                    ...
                    f_key = torch.tensor(self.attn1.fd)
                    _,L,D = self.bank[0].shape # (1+total_video_length*p)x2, L, D
                    cbank = self.bank[0].view(2, -1, L, D) # 2 x 1+total_video_length*p x L x D
                    cbank = cbank[:,1:,:,:] # 2 x total_video_length*p x L x D
                    cbank = cbank.view(2, self.attn1.p, -1, L, D)
                    cbank = [cbank[:,:,f_key,:,:].view(-1, self.attn1.p*L, D)] # 32 x (pxL) x D

                    # reference image
                    rbank = [torch.stack([self.bank[0][0], self.bank[0][int(self.bank[0].shape[0]/2)]], dim=0)]
                    rbank = [rearrange(d.unsqueeze(1).repeat(1, video_length, 1, 1), "b t l c -> (b t) l c")[:hidden_states.shape[0]] for d in rbank]

                    if self.attn1.cal:
                        # mask: pose x (pose, ref image, cal image)

                        B, Np, D = norm_hidden_states.shape
                        _, Nc, D = cbank[0].shape
                        attention_mask = torch.zeros((B,Np,(Np+Np+Nc)), dtype=norm_hidden_states.dtype, device=norm_hidden_states.device)
                        # tp: total person, cc: current person
                        for cc in range(int(Nc/Np)):
                            cal_mask = cal_mask_generate(norm_hidden_states, cbank[0], step=self.attn1.t, fd=self.attn1.fd, raw_mask=self.attn1.c_mask, p_mask_=self.attn1.p_mask, tp=self.attn1.p, cp=cc)
                            attention_mask[:,:,(2+cc)*Np:(2+cc)*Np+Np] = cal_mask
                        
                        ref_mask = ref_mask_generate(norm_hidden_states, cbank[0], step=self.attn1.t, fd=self.attn1.fd, raw_mask=self.attn1.c_mask, p_mask_=self.attn1.p_mask, tp=self.attn1.p, cp=cc)
                        attention_mask[:,:,Np:2*Np] = ref_mask

                        # attention
                        hidden_states_uc = self.attn1(norm_hidden_states, 
                                                    encoder_hidden_states=torch.cat([norm_hidden_states] + rbank + cbank, dim=1),
                                                    attention_mask=attention_mask) + hidden_states
                    else:
                        # mask: pose x (pose, ref image)
                        B, Np, D = norm_hidden_states.shape
                        #attention_mask = torch.zeros((B,Np,(Np+Np)), dtype=norm_hidden_states.dtype, device=norm_hidden_states.device)

                        # attention
                        hidden_states_uc = self.attn1(norm_hidden_states, 
                                                    encoder_hidden_states=torch.cat([norm_hidden_states] + rbank, dim=1),
                                                    attention_mask=attention_mask) + hidden_states
                    ...
                    if self.attn2 is not None:
                        # Cross-Attention # with null text
                        norm_hidden_states = (
        ...
        # no use for animation pipeline and image encoder
